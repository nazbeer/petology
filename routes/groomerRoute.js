...
const Appointment = require("../models/openGroomingModel");
const mobAppointment = require("../models/mobgroomappModel");
...
router.get("/get-all-appointments", authMiddleware, async (req, res) => {
  try {
    const appointmentList = await UserappModel.find({module: 'grooming'})
...
router.get("/get-all-mob-appointments", authMiddleware, async (req, res) => {
  try {
    const appointmentList = await UserappModel.find({module: 'mobile_grooming'})
      // .populate('doctors', 'name') // Assuming 'doctor' field is a reference to User model
      // .populate('petlists', 'name') // Assuming 'pet' field is a reference to Pet model
      .exec();
      const userId = appointmentList.map((item) => item.userId);
